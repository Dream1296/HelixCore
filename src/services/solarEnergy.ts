    ...
    myEvent.addListener('mqtt_ack', (e: mqtt_message) => {
        if (e.topic == '/se/esp8266_01/data') {
            ...
            const sql = `
                ...
                date, ch1_u, ch1_i, ch2_u, ch2_i, pwm, wifiRSSI, time
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                `;
            // dbSql(sql, [formatDateToSQL(date), data.ch1.U, data.ch1.I, data.ch2.U, data.ch2.I, data.pwm, data.wifiRSSI, data.time]
            //     , undefined, 'feel'
            // );
        }
